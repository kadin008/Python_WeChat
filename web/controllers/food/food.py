...
def index():
    ...
    query = Food.query
    if 'mix_kw' in req:
        rule = or_(Food.name.like('%{0}%'.format(req['mix_kw'])), Food.tags.like('%{0}%'.format(req['mix_kw'])))
    ...
    cat_mapping = getDicFilterField(FoodCat, FoodCat.id, 'id', [])
...
def set():
    ...
    price = req['price'] if 'price' in req else None
    main_image = req['main_image'] if 'main_image' in req else None
    summary = req['summary'] if 'summary' in req else None
    stock = int(req['stock']) if 'stock' in req else None
    tags = req['tags'] if 'tags' in req else None
